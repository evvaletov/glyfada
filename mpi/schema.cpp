...
static void loadNode (int __rk_sched) {
  ...
  /* ATT: name*/
  node.rk = eo::mpi::Node::getRankFromName (getAttributeValue ("name"));
  /* ATT: num_workers */
...
static void loadGroup () {
  ...
  /* ATT: scheduler*/
  int rk_sched = eo::mpi::Node::getRankFromName (getAttributeValue ("scheduler"));
...
void loadSchema (const char * __filename) {
  ...
  /* Looking for my node */
  for (unsigned i = 0; i < the_schema.size (); i ++)
    if (the_schema [i].rk == eo::mpi::Node::getNodeRank ())
